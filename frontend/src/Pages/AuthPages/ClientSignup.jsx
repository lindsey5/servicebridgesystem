...
import useClientSignupReducer from '../../hooks/useClientSignupReducer';
...
const signup = async ({e, state, dispatch, confirmPass, navigate}) =>{
    ...
    if(state.password !== confirmPass){
        ...
        try{
            const response = await fetch('https://servicebridgesystem.onrender.com/client-signup',{
                method: 'POST',
                headers: {
...
const ClientSignup = () => {
    const { state, dispatch } = useClientSignupReducer();
